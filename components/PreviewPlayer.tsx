...
const PreviewPlayer: React.FC<PreviewPlayerProps> = ({ videoUrl, timeRange, cropArea }) => {
    ...
    const [videoStyle, setVideoStyle] = React.useState<React.CSSProperties>({});
    ...
    }, [timeRange.start, videoUrl, setCurrentTime]);
    ...
    const onVideoTimeUpdate = () => handleTimeUpdate(videoRef.current);
    const onPlayPause = () => handlePlayPause(videoRef.current);
    const onScrubberChange = (e: React.ChangeEvent<HTMLInputElement>) => handleScrubberChange(e, videoRef.current);
            ...
            <div className="bg-black rounded-lg overflow-hidden relative flex flex-col flex-grow">
                ...
                <div className="p-3 bg-gray-900/50 space-y-2">
                    <div className="flex items-center gap-3">
                        <button onClick={onPlayPause} className="p-2 bg-gray-700 rounded-full hover:bg-gray-600">
